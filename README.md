...
- **RESP 协议**：完整的 RESP 命令构建（`buildRESPCommand`）与响应解析（`RESP_Parser`），支持 Simple String / Error / Integer / Bulk String / Array / Null，并处理不完整数据（`IncompleteRESPException`）。
- **异步网络框架**：`EpollContext` 基于 Linux epoll + eventfd 实现多事件循环线程，非阻塞读写、自动分包/粘包处理（仅支持 Linux / WSL2）。
- **高层客户端**：`SimpleRedisClient` 内置 4 条连接（`DEFAULT_CLIENT_COUNT = 4`），按线程局部计数轮询分发请求，支持 pipeline（批量发送后统一等待响应）。
- **线程池**：自研 `ThreadPool`（含自旋锁 `SpinLock`、任务队列调度、`Result` 结果回传与异常捕获）。
- **日志系统**：`Logger`，支持 DEBUG / INFO / WARNING / ERROR 级别。
...
test/
  ...
  test_thread_pool_benchmark.cpp 线程池正确性 + 性能基准
...
| `test_redis_bench` | SET 压力测试，参数：命令条数 N（默认 200000） |
| `test_redis_client_acc` | pipeline 压力测试（1,000,000 条 SET，可调） |
| `test_redis_acc` | RESP 解析正确性 + 异步集成测试 |
| `test_thread_pool` | 线程池正确性 + 性能基准 |
...
- 编译器：`gcc-14 / g++-14`（`CMakeLists.txt` 中已固定路径 `/usr/bin/gcc-14`、`/usr/bin/g++-14`）。
- CMake ≥ 3.10。
- 一个正在运行的 Redis 服务（默认连接 `127.0.0.1:6379`）。

本机开发环境：WSL2 中代码位于 `/home/computer/project/Demo/`，Redis 运行在 Windows 侧 `127.0.0.1:6379`（WSL2 localhost 转发可直接访问）。
...
# 以 Release 为例（Debug / RelWithDebInfo 同理）
...
# Redis 压力测试（需先启动 Redis）
./cmake-build-relwithdebinfo-wsl/test_redis_bench 200000     # 默认 200000 条 SET
./cmake-build-relwithdebinfo-wsl/test_redis_client_acc       # pipeline：1000 批 × 1000 条
```

# 测试结果(Benchmark Result)

测试环境 / Environment：WSL2 (Ubuntu 20.04)，20 逻辑核心，`gcc-14 / g++-14`，Redis 5.0.14.1（Windows，`127.0.0.1:6379`）。
...
| 失败 Failures | 0 |
| 耗时 Elapsed | 2.53 s |
| 吞吐 Throughput | **~394,521 ops/s** |

### 2. Redis 异步 SET 压力测试（`test_redis_bench`）

单连接 + epoll 异步读写，N = 10,000：

| 指标 | 结果 |
| --- | --- |
| 发送命令 Commands sent | 10,000 |
| 成功 / 失败 | 10,000 / 0 |
| 耗时 Elapsed（含测试固定 1s 等待） | 1.01 s |
| 吞吐 Throughput（程序输出） | ~9,912 req/s |

> 备注：该测试的计时包含一个固定的 1 秒轮询等待，实际传输耗时远小于显示值；在本环境（WSL2 ↔ Windows 版 Redis）下 N ≤ 10,000 可稳定通过，N ≥ 100,000 时偶发将 2048B 固定读缓冲中的残留字节误解析为响应（约 8 条），属该测试用例在读缓冲边界上的已知问题。

### 3. 线程池性能基准（`test_thread_pool`，Release）

| 场景 | 参数 | 耗时 | 吞吐 |
| --- | --- | --- | --- |
| 空任务 Empty | 8 线程 × 1,000,000 任务 | 61.21 ms | ~16.3M tasks/s |
| CPU 密集 | 8 线程 × 1,000,000 任务 × 100 次迭代 | 865.46 ms | ~1.16M tasks/s |
| I/O 模拟（sleep 5ms） | 8 线程 × 100 任务 | 66.48 ms | ~1,504 tasks/s |
| 空任务最佳点 | 2 线程 × 100,000 任务 | 2.34 ms | ~42.8M tasks/s |

正确性测试（int / double / string / vector / 异常 / 并发结果 / 边界值）全部 **PASSED**。

### 4. 正确性测试（`test_redis_acc`）

RESP 命令构建、响应解析（简单字符串 / 错误 / 整数 / 批量字符串 / 空值 / 数组 / 不完整数据）以及异步 Redis 集成测试全部通过。
